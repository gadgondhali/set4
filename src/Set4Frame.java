...
public class Set4Frame extends JFrame implements ActionListener, Observer {  // Constructor
  public Set4Frame(String title, Set4 theApp)
  { 
...
*/
  ...
  private File showDialog(String dialogTitle,
    ...
    files.rescanCurrentDirectory();
    files.setSelectedFile(file);
    int result = files.showDialog(Set4Frame.this, null);  // Show the dialog
    ...
    if(file == null) {
      ...
      if(file == null ||  (file.exists() &&    // Check for existence
         JOptionPane.NO_OPTION ==              // Overwrite warning
              JOptionPane.showConfirmDialog(Set4Frame.this,
  ...
  private void saveSketch(File outFile) {
    try {
      ...
      JOptionPane.showMessageDialog(Set4Frame.this,
  ...
  public void openSketch(File inFile) {
    try {
      ...
      theApp.insertModel((Set4Model)in.readObject());
      in.close();
      modelFile = inFile;
      filename = modelFile.getName();            // Update the file name
      setTitle(frameTitle+modelFile.getPath());  // Change the window title
  ...
  class FileAction extends AbstractAction {    
    ...
    FileAction(String name) {
      ...
      String icf[]={"New.gif","Open.gif","Save.gif","Print.gif","Line.gif","Rectangle.gif","Circle.gif",
    		        "Ellipse.gif","Curve.gif","Text.gif","Blue.gif","Red.gif","Yellow.gif","Green.gif","Fill.gif"};
      int i;
      for(i=0;i<icf.length;i++) if(icf[i].contains(iconFileName)) break;
      if(i>=icf.length) i=icf.length-1;
      ImageIcon ic=new ImageIcon(getClass().getResource(icf[i]));
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if(name.equals(saveAction.getValue(NAME))) {
       ...
       return;
        ...
        checkForSave();
        theApp.insertModel(new Set4Model());    // Insert new empty sketch
